...
st.markdown("# Boite à moustache des du compound")
st.sidebar.header("Boite a moustache compound")
st.write(
    """Representation de l'évolution des médiannes du 'coumpound' utiliser pour évaluer les sentiments """
)
...
for _ in iter(int, 1):
    with placeholder.container():
        ...
        fig, ax = plt.subplots()
        sns.boxplot(data=df, y='compound')
        st.pyplot(fig)
        progress_bar.empty()
        time.sleep(5)
